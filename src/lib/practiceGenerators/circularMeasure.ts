...
export function generateCircularMeasureProblem(input: {
  ...
  const theta = piFracToNumber(thetaFrac);
  ...
  if (kind === 'arc_length_inverse_radius') {
    ...
    promptText = `A sector has arc length s = ${asFixed2(s)} and central angle θ = ${thetaRadPlain} rad. Find the radius r.`;
    promptKatex = String.raw`\text{A sector has }s=${asFixed2(s)},\;\theta=${thetaRadLatex}\text{ rad}.\;\text{Find }r.`;
  ...
  if (kind === 'midpoint_shaded_area_inverse_radius') {
    ...
    promptText = `In a sector, C and D are the midpoints of OA and OB. If ∠AOB = ${thetaDecLatex} radians and the shaded area is approximately ${asFixed2(area)}, find the radius r.`;
    promptKatex = String.raw`\text{C and D are midpoints of }OA\text{ and }OB.\;\angle AOB=${thetaDecLatex}\text{ rad}.\\\text{If shaded area }\approx ${asFixed2(area)}\text{, find }r.`;
  ...
  if (kind === 'sector_area_inverse_radius') {
    ...
    promptText = `A sector has area A = ${aPlain} and central angle θ = ${thetaRadPlain} rad. Find the radius r.`;
    promptKatex = String.raw`\text{A sector has }A=${aLatex},\;\theta=${thetaRadLatex}\text{ rad}.\;\text{Find }r.`;
  ...
  if (kind === 'segment_area_forward') {
    ...
    if (input.difficulty === 'hard') {
      steps.push({ katex: eqSeg, text: 'A segment area is sector area minus triangle area. The compact formula is:' });
      steps.push({ katex: eqSeg, text: 'Make sure θ is in radians, then substitute r and θ.' });
      steps.push({ katex: String.raw`\text{The chord is drawn here to show the triangle boundary used in the formula.}`, text: 'Chord is relevant for segment diagrams.' });
      steps.push({ katex: String.raw`A_{\text{segment}} = \frac{1}{2}\cdot ${r}^2\left(${thetaRadLatex} - \sin(${thetaRadLatex})\right)`, text: 'Substitute.' });
      steps.push({ katex: String.raw`A_{\text{segment}} \approx ${asFixed2(seg)}`, text: 'Calculate and round to 2 decimal places.' });
    } else {
      steps.push({ katex: eqSeg, text: 'Use the segment area formula (sector minus triangle).' });
      steps.push({ katex: String.raw`\text{The chord is drawn because the segment is bounded by a chord.}`, text: 'Chord is relevant for segment diagrams.' });
  ...
  if (kind === 'segment_area_inverse_radius') {
    ...
    promptText = `A minor segment has area approximately ${asFixed2(seg)} and central angle θ = ${thetaRadPlain} rad. Find the radius r.`;
    promptKatex = String.raw`\text{A minor segment has }A_{\text{segment}}\approx ${asFixed2(seg)},\;\theta=${thetaRadLatex}\text{ rad}.\;\text{Find }r.`;

    steps.push({ katex: eqSeg, text: 'Start with the segment area formula.' });
  ...
  if (kind === 'segment_area_inverse_theta') {
    ...
    steps.push({ katex: eqSeg, text: 'Use the segment formula and solve for θ (typically by numerical methods).'});
  ...
  const diagramSvg = buildCircularMeasureDiagramSvg({
    ...
    shadeMode: kind.includes('midpoint_shaded') ? 'midpoint_shaded' : 'sector',
    labelR: kind.includes('inverse_radius') ? String.raw`r = ?` : String.raw`r = ${r}`,
  ...
  const id = stableId('circular-measure', input.seed, `${kind}-${r}-${thetaFrac.n}-${thetaFrac.d}`);
