...
export default function TypingAnswerMathInput({ value, onChange, placeholder, className, disabled, enableScripts }: Props) {
  ...
  return (
    <div
      ...
      className={`min-h-[48px] w-full rounded-md border px-3 py-1 text-base focus:outline-none focus:ring-2 focus:ring-ring ${
      ...
      onKeyDown={(e) => {
        ...
        if (scriptsEnabled) {
          ...
          // Exit scripts with normal ArrowLeft/ArrowRight when caret is at the start/end of the script.
          const isArrowRight = e.key === 'ArrowRight';
          const isArrowLeft = e.key === 'ArrowLeft';
          if (insideScript && (isArrowRight || isArrowLeft)) {
            const first = document.createRange();
            first.selectNodeContents(insideScript);
            first.collapse(true);

            const last = document.createRange();
            last.selectNodeContents(insideScript);
            last.collapse(false);

            const atStart = range.compareBoundaryPoints(Range.START_TO_START, first) === 0;
            const atEnd = range.compareBoundaryPoints(Range.END_TO_END, last) === 0;
