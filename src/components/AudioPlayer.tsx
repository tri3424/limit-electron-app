...
type Props = {
	...
	onPositionMsChange?: (payload: { currentTimeMs: number; durationMs: number }) => void;
...
export default function AudioPlayer({
	...
	const clipStopSec = useMemo(() => {
		...
		return Math.max(0, clipEndSec);
	}, [clipActive, clipEndSec]);
	...
	useEffect(() => {
		...
		const emitPositionMs = () => {
			const currentSec = Number.isFinite(el.currentTime) ? el.currentTime : 0;
			const durationSec = Number.isFinite(el.duration) ? el.duration : 0;
			const currentTimeMs = Math.max(0, Math.floor(currentSec * 1000));
			const durationMs = Math.max(0, Math.floor(durationSec * 1000));
			onPositionMsChange?.({ currentTimeMs, durationMs });
		};

		const clampToClipWindow = (mode: 'seek' | 'time' | 'play') => {
			if (!clipActive) return;
			if (typeof clipStartSec !== 'number' || typeof clipEndSec !== 'number') return;
			...
			if (el.currentTime < clipStartSec) {
				try {
					el.currentTime = clipStartSec;
				} catch {
					// ignore
			...
			if (el.currentTime > clipEndSec || (mode !== 'seek' && el.currentTime >= clipEndSec)) {
				try {
					el.pause();
		...
		const handleTime = () => {
			clampToClipWindow('time');
			if (clipActive && typeof clipStopSec === 'number' && Number.isFinite(el.currentTime) && el.currentTime >= clipStopSec) return;
		...
		el.addEventListener('seeked', handleSeeking);

		const tick = () => {
			if (clipActive && !el.paused) {
				clampToClipWindow('time');
			}
			// Always emit position in rAF so external consumers can track scrubs/jumps precisely.
			emitPositionMs();
		...
		return () => {
			...
			el.removeEventListener('seeked', handleSeeking);
			if (rafIdRef.current !== null) {
				window.cancelAnimationFrame(rafIdRef.current);
				rafIdRef.current = null;
			}
		};
	}, [clipActive, clipStartSec, clipEndSec, clipStopSec, onEnded, onLoadedMetadata, onPlay, onPause, onTimeUpdate, seeking]);
