								...
								{adminSearchMatches.slice(0, 50).map((c) => (
									...
									>
										<div className="w-full">
											...
											<div className="text-xs text-muted-foreground">
												{formatMsAsTimestamp(Math.max(0, c.startMs)).split('.')[0]}–{formatMsAsTimestamp(Math.max(0, c.endMs)).split('.')[0]}
											</div>
						...
						<div className="space-y-4">
							...
							<div>
								...
								<div className="whitespace-pre-wrap border rounded-md bg-muted/30 p-4 text-base md:text-lg leading-relaxed max-h-[340px] overflow-y-auto overflow-x-hidden">
									{(songSrtBySongId.get(viewTarget.id) || []).length ? (
										<>
											<AudioPlayer
												src={viewTarget.audioFileUrl}
												title={viewTarget.title}
												onPositionMsChange={({ currentTimeMs }) => setViewPositionMs(currentTimeMs)}
											/>
											<div className="mt-3">
												<SongTimedLyrics cues={songSrtBySongId.get(viewTarget.id) || []} positionMs={viewPositionMs} lyricsText={viewTarget.lyrics} songTitle={viewTarget.title} />
											</div>
										</>
									) : (
										viewTarget.lyrics
						...
						<Button
							...
							onClick={async () => {
								...
								try {
									...
									await db.songSrtCues.where('songId').equals(deleteTarget.id).delete();
